...
const Profile = () => {
  ...
  const loadUserInformation = async () => {
    let request = null;  
    try{
      request = await fetch(`https://localhost:7004/api/authority/user/${userId.toLocaleLowerCase()}`, {
        method: "GET",
        headers: {
  ...
  return (
    <div style={{textAlign: "center", margin: "1rem 0", fontFamily: "Lisu Bosa"}}>
      <h1 className='color-gray'>Profile Page</h1>
      {user && (role === "admin") ?
        <h2>Username: {user.username}</h2>
      :<>
        <h2><b>Name:</b> {user && user.name}</h2>
        <h2><b>Email:</b> {user && user.email}</h2>
      </>
      }
